...
class Experto
{
    ...
    {
        try {
            $query = $this->dbh->prepare('select * from expertos where usuario="'.$usuario.'" and password=PASSWORD("'.$pass.'")');
            $query->execute();
    ...
    {
        try {
            $query = $this->dbh->prepare('insert into expertos values(null,?,?,PASSWORD(?),?,0)');
